...
Markov chain \( X_n \).
  ...
  $X_0$.  
\item The \defn{distributing matrix}\index{distributing
  matrix} $U$ is the $v \times k$ matrix with entries
\[
  U_{ij} =
  \begin{cases}
...
to $1$.
\item Let the \defn{collecting
  matrix}\index{collecting matrix} $V$ be the $k \times v$ matrix with the $j$th column,
...
two useful matrices. The \defn{distributing matrix}\index{distributing
  matrix} $U$ is the $v \times k$ matrix with entries
\[
  U_{ij} =
  \begin{cases}
